...
import { validateCommand, runCommand, type Verdict } from "@/app/lib/sandbox";
...
export default function Home() {
  ...
  const [terminalLogs, setTerminalLogs] = useState<string[]>([
    ...
    "Type 'help' to see sandbox commands. Awaiting commands...",
  ]);
  ...
  // Validate a command and, if safe, stage it for explicit confirmation.
  // Blocked commands are reported and never become runnable ("check before
  // running"). Used by the terminal input and the file-op buttons alike.
  const queueCommand = useCallback(
    (raw: string) => {
      const verdict = validateCommand(raw);
      if (verdict.status === "blocked") {
        addLog(`BLOCKED: ${verdict.command}`);
        addLog(`ERROR: ${verdict.reason ?? "command rejected"}`);
  ...
  const cancelPending = useCallback(() => {
    setPendingVerdict((prev) => {
      if (prev) addLog(`CANCELLED: ${prev.command}`);
      return null;
    });
  }, [addLog]);

  // Execute the confirmed command against the in-memory file system only.
  const runPending = useCallback(() => {
    if (!pendingVerdict || !stack) return;
    const verdict = pendingVerdict;
    setPendingVerdict(null);
    addLog(`$ ${verdict.command}`);

    const result = runCommand(verdict.argv, fileContents, { stack });

    if (result.clear) {
      setTerminalLogs(["Console cleared."]);
    }
    result.logs.forEach((line) => addLog(line));
    if (result.files) setFileContents(result.files);
    if (result.openFile) setOpenFilePath(result.openFile);
    if (result.deploy) void handleDeploy();
  }, [pendingVerdict, stack, fileContents, addLog, handleDeploy]);
    ...
    <div className="flex min-h-screen w-full flex-col bg-white text-black font-sans box-border selection:bg-[#16CAC6] selection:text-black">
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
            ...
            <div>
              ...
              <div className="mb-2 flex gap-1">
                <button
                  type="button"
                  onClick={() => {
                    const path = window.prompt("New file path (project-relative):");
                    if (path && path.trim()) queueCommand(`touch ${path.trim()}`);
                  }}
                  className="flex-1 border-2 border-black bg-white px-2 py-1 text-[10px] font-bold uppercase hover:bg-[#16CAC6]"
                >
                  + File
                </button>
                <button
                  type="button"
                  onClick={() => {
                    const path = window.prompt("New folder path (project-relative):");
                    if (path && path.trim()) queueCommand(`mkdir ${path.trim()}`);
                  }}
                  className="flex-1 border-2 border-black bg-white px-2 py-1 text-[10px] font-bold uppercase hover:bg-[#16CAC6]"
                >
                  + Folder
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-white">
          ...
          {/* Tabs */}
          <div className="flex h-10 brutal-border-b bg-[#e5e5e5] shrink-0 items-stretch justify-between">
            ...
            {openFilePath ? (
              <div className="flex items-center gap-1 pr-2">
                <button
                  type="button"
                  onClick={() => {
                    const dst = window.prompt("Move/rename to (project-relative path):", openFilePath);
                    if (dst && dst.trim() && dst.trim() !== openFilePath) {
                      queueCommand(`mv ${openFilePath} ${dst.trim()}`);
                    }
                  }}
                  className="border-2 border-black bg-white px-2 py-1 text-[10px] font-bold uppercase hover:bg-[#16CAC6]"
                >
                  Move
                </button>
                <button
                  type="button"
                  onClick={() => queueCommand(`rm ${openFilePath}`)}
                  className="border-2 border-black bg-white px-2 py-1 text-[10px] font-bold uppercase hover:bg-red-400"
                >
                  Delete
                </button>
              </div>
          ...
          {/* Terminal / Output */}
          <div className="h-64 lg:h-72 brutal-border-t bg-black text-[#16cac6] flex flex-col shrink-0">
            <div className="flex items-center justify-between px-4 py-1 border-b border-gray-800 shrink-0">
              <span className="text-xs font-mono font-bold text-white uppercase tracking-wider">Sandbox Console</span>
              <button
                type="button"
            ...
            <div className="flex-1 overflow-y-auto p-4 font-mono text-sm">
              {terminalLogs.map((log, i) => (
                <div key={i} className="mb-1 leading-snug whitespace-pre-wrap">
                  {log.startsWith("[") ? (
                    <>
            ...
            {pendingVerdict ? (
              <div className="shrink-0 border-t border-gray-700 bg-[#10201f] px-4 py-2">
                <p className="text-[11px] uppercase tracking-wider text-yellow-300 font-bold">
                  Confirm command
                </p>
                <code className="block break-all text-sm text-white">$ {pendingVerdict.command}</code>
                <div className="mt-2 flex gap-2">
                  <button
                    type="button"
                    onClick={runPending}
                    className="border-2 border-[#16CAC6] bg-[#16CAC6] px-3 py-1 text-[11px] font-bold uppercase text-black hover:opacity-90"
                  >
                    Run
                  </button>
                  <button
                    ...
                    className="border-2 border-gray-500 px-3 py-1 text-[11px] font-bold uppercase text-gray-300 hover:bg-gray-700"
                  >
                    Cancel
                  </button>
                </div>
            ...
            {/* Command input */}
            <form
              onSubmit={submitCommandInput}
              className="flex shrink-0 items-center gap-2 border-t border-gray-800 px-4 py-2"
            >
              <span className="font-mono text-sm font-bold text-[#16CAC6]">$</span>
              <input
                type="text"
                value={commandInput}
                ...
                spellCheck={false}
                autoComplete="off"
                placeholder={pendingVerdict ? "confirm or cancel the pending command above" : "try: help · algokit generate token · ls contracts"}
                disabled={!!pendingVerdict}
                className="flex-1 bg-transparent font-mono text-sm text-white outline-none placeholder:text-gray-600 disabled:opacity-50"
              />
            </form>
          </div>
